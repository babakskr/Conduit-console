...
# * **Fix:** Default mode now correctly applies PRI 10 (Docker) and 15 (Native) when no arguments provided.
# * **Logic:** If arguments are passed, mixed mode is supported (manual override vs skip).
# </component_release_notes>
...
# Init inputs as Unset (-1) to distinguish from User entering "0"
INPUT_DOCKER_PRI=-1
INPUT_NATIVE_PRI=-1
VERBOSE=0
...
# --- LOGIC: APPLY DEFAULTS OR MANUAL ---
# If both inputs are still -1 (User gave no args), Load Defaults
if [[ $INPUT_DOCKER_PRI -eq -1 && $INPUT_NATIVE_PRI -eq -1 ]]; then
    ...
    # If user set some args but not others, treat unset (-1) as 0 (Skip)
...
echo -e "${GREEN}Starting Optimizer v1.7.0${NC}"
echo "---------------------------------------------------"
...
# ... (Rest of execution logic remains the same, removed for brevity but assume full execution code here) ...
# [Copy the Phase 1 & Phase 2 execution blocks from previous version here]
# ...
