...
# * **Security:** Restricted PRI input range to 5-20 (Prevented unsafe PRI 1).
# * **Standard:** Added `-ver` flag to show version info.
# * **CLI:** Validates input range strictly; exits if inputs are unsafe.
# </component_release_notes>
...
show_version() {
    ...
    echo "Repo: https://github.com/babakskr/Conduit-console"
}
...
show_help() {
    ...
    echo "  -dock <val>    Set Docker PRI (5-20). 0 to Skip."
    echo "  -srv  <val>    Set Native PRI (5-20). 0 to Skip."
    echo "  -v             Verbose mode."
...
validate_range() {
    local val=$1
    local name=$2
    # 0 is allowed (Skip), otherwise must be 5-20
    if [[ $val -ne 0 ]]; then
        if [[ $val -lt 5 || $val -gt 20 ]]; then
            echo -e "${RED}[ERROR] $name Priority must be between 5 and 20.${NC}"
            echo "       (Value 1-4 is too dangerous for system stability)"
            exit 1
        fi
    fi
}
...
echo -e "${GREEN}Starting Optimizer v1.8.0${NC}"
echo "---------------------------------------------------"
...
# ... (Execution logic same as stable version) ...
# PHASE 1: DOCKER
TOTAL_SUCCESS=0
